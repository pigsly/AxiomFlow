...
- 呈現層應遵守軟性 `PDR` 摘要規則
- 必須使用 status panel 呈現，layout 應遵守 `render_pdr_status` 的欄位配置
...
其中 `PDR Summary` 應優先採用近似 `codex cli /status` 的區塊式 layout。
...
如果使用 `render_pdr_status` skill，預設 layout 應為：
...
必須使用 `rich_panel`，則應輸出單一 bordered status box，欄位不可任意省略。
