...
## Why not just let the agent infer missing details?

Because inference under weak alignment is one of the main failure modes in AI-assisted engineering.

The problem is not that the agent cannot infer. The problem is that the team cannot reliably control when those inferences are safe.
...
Direction may evolve with a milestone. Boundaries should remain stable unless humans explicitly change them.
