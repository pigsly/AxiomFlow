...
### `PDR` Output Obligation

After every `PDR`, a human-readable summary layout must be produced.

This output:

- is a required standard presentation of the `PDR` result and must not be omitted
- is used to show alignment level, risk signals, and review highlights
- does not replace the formal conflict handling and stop rules in Section 6
- should follow the soft `PDR` summary rules at the presentation layer
- if rendered as a status panel, should follow the field layout defined by `render_pdr_status`

In other words:

- `Governance.md` defines what `PDR` must check and when execution must stop
- `Governance.md` defines how `PDR` results should be summarized and presented
- `render_pdr_status` defines how that summary should be rendered into a terminal status layout

### Requirement and Spec Checks

- each `SPEC-XXX.md` must have a resolvable `related_req`
- `SPEC` must not exceed its corresponding `REQ`
...
- `SPEC` must not conflict with `ADR`
- if supporting ADR files are referenced, check whether they still align with the primary `ADR`
- `SPEC` must not conflict with `CONTRACT`
- check whether `ADR`, `CONTRACT`, and `SPEC` conflict with each other or leave unclear boundaries

### Standard `PDR` Output Format

After `PDR` completes, the default output must contain these two parts:

1. `PDR Summary`
2. `Summary List`

`PDR Summary` must use `.codex/skills/render_pdr_status` and must use a `rich_panel` bordered status box layout.
...
`Summary List` should then provide the most important reviewer-facing points in bullets.

`render_pdr_status` is the standard renderer for `PDR Summary`. It must use `rich_panel`, output a single bordered status box, and must not omit fields arbitrarily.

`Summary List` remains part of the standard `PDR` output, but it is outside the responsibility of the `render_pdr_status` skill.
If a workflow requires `Summary List`, it should be produced by the upstream `PDR` layer or another summary renderer.

### `PDR` Output Tone Rules

The tone of `PDR Summary` and `Summary List` should follow soft `PDR` principles:

- prioritize readable signals instead of turning the summary itself into execution-time control
- avoid using `BLOCK`, `FAIL`, `PASS`, `DENY`, or `REJECTED` in the summary block
- prefer `OK`, `Review Suggested`, `Needs Clarification`, and `Conflict Detected`

Note:

- a non-blocking tone at the summary layer does not mean the execution layer may ignore conflicts
- whenever Section 6 conditions are met, block, warn, and stop-and-wait behavior is still required
...
At the same time, `PDR Summary` and `Summary List` should still be output so that humans can quickly see in the CLI or text result:

- the overall signal
- the main risk location
- which document relationship should be reviewed first
